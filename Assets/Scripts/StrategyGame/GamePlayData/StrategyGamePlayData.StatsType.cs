...
public partial class StrategyGamePlayData // StatsType
{
	...
	// 유닛 피해량 (치명)
	// 기본 피해량 = (유닛_공격력 - 유닛_방어력)
	// 치명 피해량 = ((기본 피해량 + 유닛_치명공격력) * 치명피해율 - 유닛_치명방어력)
	...
	// 타겟 변경 => 유닛_조준지연시간_c => (유닛_연속공격횟수 * 유닛_연속공격지연시간_c)(공격이벤트 발생) => 유닛_재공격지연시간_c => 재공격
	// 평균 DPS
	// 시간 = (유닛_연속공격횟수 * 유닛_연속공격지연시간_c) + 유닛_재공격지연시간_c 
	...
	public enum StatsType : ushort // 0 ~ 65,535
	{
		...
		유닛_명중피격수        = 1122, // 투사체에 1회 명중할 경우, 한번에 몇번의 피해를 발생시키는지 (0 이면 피해 계산 없음)
		유닛_연속공격횟수      = 1123, // 1번이 공격기회에서 연속으로 총을 발사하는 횟수(0 이면 공격안함) (연발)
		유닛_동시공격개수      = 1124, // 1번 총알이 발사될떄 한번에 나가는 투사체 개수(0 이면 공격안함) (산탄)
