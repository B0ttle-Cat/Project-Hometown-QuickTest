...
public class UnitProfileObject : ScriptableObject, ITargetToPanelAPI
{
	...
	void SetTestStatsValue()
	{
		...
		// 전투_공격소모_물자, 유닛_공격소모_전력은 '공격 비용'에 해당합니다.
		stats.GetCost.AttackCostMaterial = 1;
		stats.GetCost.AttackCostElectric = 1;

		// --- 🛡️ Common (공통) ---
		stats.GetCommon.MaxDurability = 1000;      // 전투_최대내구도
		stats.GetCommon.HealingPower = 10;         // 전투_치유력
		stats.GetCommon.RecoveryPower = 1;         // 전투_회복력
		...
		// --- 彈 Ammo (탄약) ---
		stats.GetAmmo.AmmunitionCapacity = 8;      // 전투_탄용량
		stats.GetAmmo.ConcurrentAttackCount = 1;   // 전투_동시공격개수
		stats.GetAmmo.ContinuousAttackCount = 3;   // 전투_연속공격횟수

		// --- 💥 Offense (공격) ---
		stats.GetOffense.projectileKey = ProjectileKey.일반탄_소형;

		stats.GetOffense.AttackPower = 10;           // 전투_공격력
		stats.GetOffense.CriticalAttackPower = 30;   // 전투_치명공격력
		stats.GetOffense.CriticalDamageRatio = 200;  // 전투_치명피해율

		stats.GetOffense.PenetrationLevel = 1;       // 전투_관통레벨
		stats.GetOffense.EMPImpactLevel = 1;         // 전투_EMP충격레벨
		stats.GetOffense.StatusPotencyLevel = 1;     // 전투_상태이상적용레벨

		stats.GetOffense.HitChanceScore = 70;        // 전투_공격명중기회
		stats.GetOffense.CriticalChanceScore = 30;   // 전투_치명명중기회

		// --- 🛡️ Defense (방어) ---
		stats.GetDefense.protectType = ProtectionType.일반;

		stats.GetDefense.AntiAttackPower = 1;        // 전투_방어력
		stats.GetDefense.AntiCriticalAttackPower = 10; // 전투_치명방어력

		stats.GetDefense.AntiPenetrationLevel = 1;   // 전투_장갑레벨 (Anti-Penetration)
		stats.GetDefense.AntiEMPImpactLevel = 1;     // 전투_EMP방호레벨
		stats.GetDefense.AntiStatusPotencyLevel = 1; // 전투_상태이상저항레벨

		stats.GetDefense.AntiHitChanceScore = 10;    // 전투_공격회피기회 (Anti-Hit Chance)
		stats.GetDefense.AntiCriticalChanceScore = 20; // 전투_치명회피기회 (Anti-Critical Chance)
	}
#endif
}
