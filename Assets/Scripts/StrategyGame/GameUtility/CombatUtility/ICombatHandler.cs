...
public interface ICombatCommon : IDurabilityValue, IStatsValueControl, IStrategyElement
{
	...
	int AmmunitionCapacity => GetStatsValue(StatsType.전투_탄용량);
	int AmmunitionUsed => GetStatsValue(StatsType.전투_사용탄수);
	int ConcurrentProjectileCount => GetStatsValue(StatsType.전투_동시공격개수);
	int ContinuousAttackCount => GetStatsValue(StatsType.전투_연속공격횟수);

	// ⏱️ 딜레이 스탯 (Time Delays)
	float AimDelayTime => GetStatsValuePrecent(StatsType.전투_조준지연시간_c);
	float ContinuousAttackDelayTime => GetStatsValuePrecent(StatsType.전투_연속공격지연시간_c);
	float ReattackDelayTime => GetStatsValuePrecent(StatsType.전투_재공격지연시간_c);
	float ReloadTime => GetStatsValuePrecent(StatsType.전투_재장전시간_c);

	// 💸 공격 소모 자원 스탯 (Cycle Cost)
	float AttackCostMaterial => GetStatsValue(StatsType.전투_공격소모_물자);
	float AttackCostPower => GetStatsValue(StatsType.전투_공격소모_전력);

	// 🔭 범위 스탯 (Range)
	float AttackLimitRangeMin => GetStatsValuePrecent(StatsType.전투_공격범위_종료최소_c);
	float AttackStartRangeMin => GetStatsValuePrecent(StatsType.전투_공격범위_시작최소_c);
	float AttackStartRangeMax => GetStatsValuePrecent(StatsType.전투_공격범위_시작최대_c);
	float AttackLimitRangeMax => GetStatsValuePrecent(StatsType.전투_공격범위_종료최대_c);
	float ActionRange => GetStatsValuePrecent(StatsType.전투_행동범위_c);
	float VisionRange => GetStatsValuePrecent(StatsType.전투_시야범위_c);
}
public interface ICombatOffense : IStatsValueControl, IStrategyElement
{
	...
	int AttackPower => GetStatsValue(StatsType.전투_공격력);

	// 🎯 치명타 스탯 (Critical)
	float CriticalDamageRatio => GetStatsValuePrecent(StatsType.전투_치명피해율);
	int CriticalAttackPower => GetStatsValue(StatsType.전투_치명공격력);

	// 🛡️ 관통 및 적용 스탯 (Penetration & Application)
	int PenetrationLevel => GetStatsValue(StatsType.전투_관통레벨);
	int EMPImpactLevel => GetStatsValue(StatsType.전투_EMP충격레벨);
	int StatusPotencyLevel => GetStatsValue(StatsType.전투_상태이상적용레벨);
...
public interface ICombatDefance : IDurabilityValue, IStatsValueControl, IStrategyElement
{
	...
	int AntiCriticalAttackPower => GetStatsValue(StatsType.전투_치명방어력);

	// 🛡️ 장갑 및 방호 스탯 (Armor & Protection)
	int AntiPenetrationLevel => GetStatsValue(StatsType.전투_장갑레벨);
	int AntiEMPImpactLevel => GetStatsValue(StatsType.전투_EMP방호레벨);
	int AntiStatusPotencyLevel => GetStatsValue(StatsType.전투_상태이상저항레벨);

	// 확률 회피 스탯 (Evasion Score)
	int AntiHitChanceScore => GetStatsValue(StatsType.전투_공격회피기회);
	int AntiCriticalChanceScore => GetStatsValue(StatsType.전투_치명회피기회);
