...
using UnityEngine;
[CreateAssetMenu(fileName = "KeyPairUnitName", menuName = "Scriptable Objects/KeyPairAssets/KeyPairUnitName")]
public class KeyPairUnitName : KeyPairAssets<StrategyGamePlayData.UnitKey, string>
{
	public static KeyPairUnitName Load(Language.Type type, string name)
	{
		string path = $"{nameof(KeyPairUnitName)}/{type.ToString()}/{name}";
		Debug.Log(path);
		return Resources.Load<KeyPairUnitName>(path);
	}
